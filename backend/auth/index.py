...
def handler(event: dict, context):
    ...
    try:
        if method == 'GET':
            ...
            cur.execute(
                f"SELECT u.id, u.name, u.email, u.balance, u.created_at "
        ...
        if action == 'register':
            ...
            cur.execute(
                ...
                f"RETURNING id, name, email, balance, created_at",
                (name, email, pw_hash),
            )
            row = cur.fetchone()
        ...
        if action == 'login':
            ...
            password = body.get('password') or ''
            cur.execute(
                f"SELECT id, name, email, balance, created_at, password_hash "
            ...
            token = create_session(cur, row[0])
            conn.commit()
            return resp(200, {'token': token, 'user': user_dict(row[:5])}, headers_common)
